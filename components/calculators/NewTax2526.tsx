...
export function NewTax2526() {
  ...
  const handle = () => { computeAndStore(income); trackCalculate('new-income-tax-2526'); };
    ...
    <div className="grid grid-cols-1 lg:grid-cols-[1fr_1fr_280px] gap-4 items-start">
      ...
      <div className="bg-white rounded-2xl border border-slate-200 p-3 space-y-2.5">
        ...
        <div className="bg-slate-50 rounded-xl p-3 space-y-1 text-[11px]">
          <p className="font-bold text-slate-600 text-[10px] uppercase tracking-wider mb-1.5">Tax Slabs</p>
          {[
            ['₹0 – ₹4L', '0%'], ['₹4L – ₹8L', '5%'], ['₹8L – ₹12L', '10%'],
            ['₹12L – ₹16L', '15%'], ['₹16L – ₹20L', '20%'],
            ['₹20L – ₹24L', '25%'], ['Above ₹24L', '30%'],
          ].map(([slab, rate]) => (
        ...
        {result ? (
          <>
            <div className={`rounded-2xl p-4 ${result.rebate87A > 0 ? 'bg-green-600' : 'bg-primary'} text-white`}>
              ...
              {result.rebate87A > 0 && (
                <p className="text-xs mt-1 opacity-90">87A Rebate applied · Zero tax up to ₹12.75L gross!</p>
              )}
            </div>
            ...
            <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden">
              ...
              <table className="w-full text-[11px]">
                <thead className="bg-slate-50 text-slate-500">
                  <tr>
                    {['Component', 'Amount'].map(h => <th key={h} className="px-3 py-2 text-left font-semibold">{h}</th>)}
                  </tr>
                </thead>
      ...
      <ComparisonPanel records={history} emptyText="Calculate tax for different income levels to compare." />
    </div>
